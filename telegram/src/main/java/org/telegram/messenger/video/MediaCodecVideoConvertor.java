...
public class MediaCodecVideoConvertor {
    ...
    private boolean convertVideoInternal(String videoPath, File cacheFile,
                ...
                if (videoIndex >= 0) {
                    ...
                    try {
                        ...
                        if (BuildConfig.DEBUG) {
                            Log.d(TAG, "create encoder with w = " + w + " h = " + h);
                        }
                        ...
                        outputSurface = new OutputSurface(cropState, resultWidth, resultHeight, rotationValue);
                            ...
                            if (audioIndex >= 0) {
                                if (copyAudioBuffer) {
                                    ...
                                    try {
                                        ...
                                        Log.e(TAG, e.getLocalizedMessage()); //s20 ultra exception
                                        e.printStackTrace();
                                    }
                                    if (maxBufferSize <= 0) {
                                    ...
                                    if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                                        ...
                                        if (trueStartTime > 0 && videoTime == -1) {
                                            if (originalPresentationTime < trueStartTime) {
                                                doRender = false;
                                                if (BuildConfig.DEBUG) {
                                                    Log.d(TAG, "drop frame startTime = " + trueStartTime + " present time = " + info.presentationTimeUs);
                                                }
                                            } else {
                        ...
                        Log.e(TAG, "bitrate: " + bitrate + " framerate: " + framerate + " size: " + resultHeight + "x" + resultWidth);
                        Log.e(TAG, e.getLocalizedMessage());
                        e.printStackTrace();
                        error = true;
                    }
        ...
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "compression completed time=" + timeLeft + " needCompress=" + needCompress + " w=" + resultWidth + " h=" + resultHeight + " bitrate=" + bitrate);
        }
    ...
    private static String createFragmentShader(
        ...
        Log.d(TAG, "source size " + srcWidth + "x" + srcHeight + "    dest size " + dstWidth + dstHeight + "   kernelRadius " + kernelRadius);
        if (external) {
